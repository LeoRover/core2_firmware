...
uint8_t MPU9250::getAK8963CID() {
  //  uint8_t c = readByte(AK8963_ADDRESS, WHO_AM_I_AK8963);  // Read WHO_AM_I
  ...
  writeByte(MPU9250_ADDRESS, I2C_SLV0_ADDR,
            AK8963_ADDRESS |
                0x80);  // Set the I2C slave address of AK8963 and set for read.
  writeByte(MPU9250_ADDRESS, I2C_SLV0_REG,
            WHO_AM_I_AK8963);  // I2C slave 0 register address from where to
                               // begin data transfer
  writeByte(MPU9250_ADDRESS, I2C_SLV0_CTRL,
            0x81);  // Enable I2C and transfer 1 byte
  delay(10);
  uint8_t c =
      readByte(MPU9250_ADDRESS, EXT_SENS_DATA_00);  // Read the WHO_AM_I byte
  return c;
}
...
void MPU9250::accelWakeOnMotion() {
  ...
  writeByte(
      MPU9250_ADDRESS, INT_ENABLE,
      0x41);  // Enable data ready (bit 0) and wake on motion (bit 6)  interrupt

  // enable wake on motion detection logic (bit 7) and compare current sample to
  // previous sample (bit 6)
  writeByte(MPU9250_ADDRESS, MOT_DETECT_CTRL, 0xC0);

  // set accel threshold for wake up at  mG per LSB, 1 - 255 LSBs == 0 - 1020
  // mg), pic 0x19 for 25 mg
  writeByte(MPU9250_ADDRESS, WOM_THR, 0x19);

  // set sample rate in low power mode
  /* choices are 0 == 0.24 Hz, 1 == 0.49 Hz, 2 == 0.98 Hz, 3 == 1.958 Hz, 4
   * == 3.91 Hz, 5 == 7.81 Hz 6 == 15.63 Hz, 7 == 31.25 Hz, 8 == 62.50 Hz, 9 =
   * 125 Hz, 10 == 250 Hz, and 11 == 500 Hz
   */
  writeByte(MPU9250_ADDRESS, LP_ACCEL_ODR, 0x02);

  c = readByte(MPU9250_ADDRESS, PWR_MGMT_1);
  writeByte(MPU9250_ADDRESS, PWR_MGMT_1,
            c | 0x20);  // Write bit 5 to enable accel cycling
...
void MPU9250::gyromagSleep() {
  ...
  writeByte(AK8963_ADDRESS, AK8963_CNTL,
            temp & ~(0x0F));  // Clear bits 0 - 3 to power down magnetometer
  temp = readByte(MPU9250_ADDRESS, PWR_MGMT_1);
  writeByte(MPU9250_ADDRESS, PWR_MGMT_1,
            temp | 0x10);  // Write bit 4 to enable gyro standby
...
void MPU9250::gyromagWake(uint8_t Mmode) {
  ...
  temp = readByte(AK8963_ADDRESS, AK8963_CNTL);
  writeByte(AK8963_ADDRESS, AK8963_CNTL,
            temp | Mmode);  // Reset normal mode for  magnetometer
  temp = readByte(MPU9250_ADDRESS, PWR_MGMT_1);
  writeByte(MPU9250_ADDRESS, PWR_MGMT_1,
            0x01);  // return gyro and accel normal mode
  delay(10);        // Wait for all registers to reset
}
...
bool MPU9250::checkNewMagData() {
  ...
  writeByte(MPU9250_ADDRESS, I2C_SLV0_ADDR,
            AK8963_ADDRESS |
                0x80);  // Set the I2C slave address of AK8963 and set for read.
  writeByte(MPU9250_ADDRESS, I2C_SLV0_REG,
            AK8963_ST1);  // I2C slave 0 register address from where to begin
                          // data transfer
  writeByte(MPU9250_ADDRESS, I2C_SLV0_CTRL,
            0x81);  // Enable I2C and transfer 1 byte
  delay(2);
  test = (readByte(MPU9250_ADDRESS, EXT_SENS_DATA_00) &
          0x01);  // Check data ready status byte
  return test;
}
...
void MPU9250::readMagData(int16_t* destination) {
  ...
  //  readBytes(AK8963_ADDRESS, AK8963_XOUT_L, 7, &rawData[0]);  // Read the six
  //  raw data and ST2 registers sequentially into data array
  writeByte(MPU9250_ADDRESS, I2C_SLV0_ADDR,
            AK8963_ADDRESS |
                0x80);  // Set the I2C slave address of AK8963 and set for read.
  writeByte(MPU9250_ADDRESS, I2C_SLV0_REG,
            AK8963_XOUT_L);  // I2C slave 0 register address from where to begin
                             // data transfer
  writeByte(MPU9250_ADDRESS, I2C_SLV0_CTRL,
            0x87);  // Enable I2C and read 7 bytes
  delay(2);
  readBytes(MPU9250_ADDRESS, EXT_SENS_DATA_00, 7,
...
void MPU9250::initAK8963Slave(uint8_t Mscale, uint8_t Mmode,
                              float* magCalibration) {
  // First extract the factory calibration for each magnetometer axis
  uint8_t rawData[3];  // x/y/z gyro calibration data stored here

  writeByte(MPU9250_ADDRESS, I2C_SLV0_ADDR,
            AK8963_ADDRESS);  // Set the I2C slave address of AK8963 and set for
                              // write.
  writeByte(MPU9250_ADDRESS, I2C_SLV0_REG,
            AK8963_CNTL2);  // I2C slave 0 register address from where to begin
                            // data transfer
  writeByte(MPU9250_ADDRESS, I2C_SLV0_DO, 0x01);  // Reset AK8963
  writeByte(MPU9250_ADDRESS, I2C_SLV0_CTRL,
            0x81);  // Enable I2C and write 1 byte
  delay(50);
  writeByte(MPU9250_ADDRESS, I2C_SLV0_ADDR,
            AK8963_ADDRESS);  // Set the I2C slave address of AK8963 and set for
                              // write.
  writeByte(MPU9250_ADDRESS, I2C_SLV0_REG,
            AK8963_CNTL);  // I2C slave 0 register address from where to begin
                           // data transfer
  ...
  writeByte(MPU9250_ADDRESS, I2C_SLV0_REG,
            AK8963_ASAX);  // I2C slave 0 register address from where to begin
                           // data transfer
  writeByte(MPU9250_ADDRESS, I2C_SLV0_CTRL,
            0x83);  // Enable I2C and read 3 bytes
  delay(50);
  readBytes(MPU9250_ADDRESS, EXT_SENS_DATA_00, 3,
            &rawData[0]);  // Read the x-, y-, and z-axis calibration values
  magCalibration[0] =
      (float)(rawData[0] - 128) / 256.0f +
      1.0f;  // Return x-axis sensitivity adjustment values, etc.
...
void MPU9250::magcalMPU9250(float* dest1, float* dest2) {
  ...
  //    Serial.println(mag_max[1]); Serial.println(mag_min[1]);
  ...
  dest1[0] = (float)mag_bias[0] * _mRes *
             _magCalibration[0];  // save mag biases in G for main program
                         ...
                         // values, +/- 14 or less deviation is a pass
{
  ...
  for (int i = 0; i < 3; i++) {
    destination[i] = 100.0f * ((float)(aSTAvg[i] - aAvg[i])) / factoryTrim[i] -
                     100.0f;  // Report percent differences
    destination[i + 3] =
        100.0f * ((float)(gSTAvg[i] - gAvg[i])) / factoryTrim[i + 3] -
        100.0f;  // Report percent differences
  }
}
...
void MPU9250::writeByte(uint8_t address, uint8_t subAddress, uint8_t data) {
  ...
  uint8_t i2cTxBuf[2];
  i2cTxBuf[0] = subAddress;
  i2cTxBuf[1] = data;
  _i2c.write(address, i2cTxBuf, 2);
}
...
void MPU9250::readBytes(uint8_t address, uint8_t subAddress, uint8_t count,
  ...
  //       buffer
  _i2c.rw(address, &subAddress, 1, dest, count);
}
