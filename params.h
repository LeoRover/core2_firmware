...
// Take into account that this value only limits the Voltage (PWM) and not the
// current.
...
// e.g. 800 means that power is limited to 80% at 0% speed, 90% at 10% speed
// etc.
const uint16_t TORQUE_LIMIT = 800;

// Input timeout in ms.
// The controller will stop the motors if it doesn't receive a command for a
// specified time. If set to 0, the controller won't check for a timeout
